...
def processo(processo_id):
    with db() as c:
        return c.execute("""
            SELECT p.*, v.vin, v.marca, v.modelo, v.motorizacao
            FROM processos p
...
def stock_placeholder():
    ...
    <p>O módulo de stock continua separado. Nesta fase estamos a consolidar o Hub Operacional.</p>
    <p><a href="/stock-home">Voltar ao menu Stock</a></p>
    """


@app.route("/artigos/novo")
def novo_artigo():
    return """
    <h2>Novo artigo</h2>
    <p>Este fluxo será religado ao módulo de stock na próxima consolidação.</p>
    <p><a href="/stock-home">Voltar ao menu Stock</a></p>
    """


@app.route("/movimentos/novo")
def novo_movimento():
    return """
...
def viaturas():
    with db() as c:
        rows = c.execute(
            "SELECT * FROM viaturas ORDER BY matricula"
        ).fetchall()
